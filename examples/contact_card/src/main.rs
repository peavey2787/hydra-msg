...
fn main() -> HydraResult<()> {
    ...
    let mut alice = Hydra::open("target/examples/contact_card/alice", "example-state")?;
    let mut bob = Hydra::open("target/examples/contact_card/bob", "example-state")?;
    ...
    let mut restored = Hydra::open("target/examples/contact_card/restored", "example-state")?;
