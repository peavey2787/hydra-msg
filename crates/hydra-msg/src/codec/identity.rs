...
const IDENTITY_SEED_KEY_LABEL: &[u8] = b"HYDRA-MSG/v2/facade/identity-seed-key";
const IDENTITY_PASSWORD_TAG_LABEL: &[u8] = b"HYDRA-MSG/v2/facade/identity-password-tag";
...
pub(crate) fn decrypt_seed(record: &IdentityRecord, password: &str) -> HydraResult<[u8; 32]> {
    ...
    let plaintext = RustCryptoBackend::aead_open(
        &key,
        &record.seed_nonce,
        b"HYDRA-MSG/v2/facade/encrypted-seed",
...
fn encrypt_seed_with_key(
    ...
    RustCryptoBackend::aead_seal(key, &nonce, b"HYDRA-MSG/v2/facade/encrypted-seed", seed)
        .map_err(Into::into)
}
