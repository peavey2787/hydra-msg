...
fn seed_key(id: IdentityId, password: &str) -> SecretBytes<32> {
...
fn decrypt_seed(record: &IdentityRecord, password: &str) -> HydraResult<[u8; 32]> {
    verify_password(record, password)?;
...
fn verify_password(record: &IdentityRecord, password: &str) -> HydraResult<()> {
...
fn encode_identity_line(record: &IdentityRecord) -> String {
    [
        "identity".to_string(),
        record.id.hex(),
...
fn encode_contact_line(contact: &HydraContact) -> String {
    [
        "contact".to_string(),
        contact.id.hex(),
...
fn encode_message_line(message: &StoredMessage) -> String {
    let mut parts = vec![
        "message".to_string(),
...
fn encode_lobby_line(lobby: &HydraLobby) -> String {
    let members = lobby
        .members
        .iter()
...
fn decode_lobby_invite(bytes: &[u8]) -> HydraResult<HydraLobby> {
    let text = std::str::from_utf8(bytes)
...
fn pack_lobby_payload(lobby_id: LobbyId, packed_message: &[u8]) -> HydraResult<Vec<u8>> {
    let mut out = Vec::new();
...
fn unpack_lobby_payload(bytes: &[u8]) -> HydraResult<(LobbyId, Vec<u8>)> {
...
fn backup_key(password: &str) -> SecretBytes<32> {
    let mut input = Vec::new();
...
fn parse_backup_outer(bytes: &[u8]) -> HydraResult<([u8; 12], Vec<u8>)> {
...
fn decode_backup(bytes: &[u8], password: &str) -> HydraResult<Vec<u8>> {
...
fn encode_identity_export(seed: &[u8; 32]) -> Vec<u8> {
    let mut out = Vec::new();
...
fn decode_identity_export(bytes: &[u8]) -> HydraResult<[u8; 32]> {
...
fn encode_contact_card(label: &str, public_key: &[u8; ML_DSA_65_VK_SIZE]) -> Vec<u8> {
...
fn decode_contact_card(bytes: &[u8]) -> HydraResult<HydraContact> {
    let text = std::str::from_utf8(bytes)
...
fn safety_code_for_contact(contact_id: ContactId) -> String {
...
fn decode_handshake_offer(bytes: &[u8]) -> HydraResult<ParsedHandshake> {
    decode_handshake(bytes, OFFER_MAGIC)
}

fn decode_handshake_answer(bytes: &[u8]) -> HydraResult<ParsedHandshake> {
    decode_handshake(bytes, ANSWER_MAGIC)
}

fn decode_handshake(bytes: &[u8], magic: &[u8]) -> HydraResult<ParsedHandshake> {
...
fn derive_facade_handshake_material(
    nonce: [u8; 32],
    left: IdentityId,
    right: IdentityId,
...
fn pack_message(message: &HydraMessage) -> HydraResult<Vec<u8>> {
    let mut out = Vec::new();
...
fn random_array<const N: usize>() -> HydraResult<[u8; N]> {
    let mut out = [0_u8; N];
    SysRng
...
fn exact_array_from_vec<const N: usize>(bytes: Vec<u8>) -> HydraResult<[u8; N]> {
    bytes
        .try_into()
...
fn hex_nibble(byte: u8) -> HydraResult<u8> {
    match byte {
        b'0'..=b'9' => Ok(byte - b'0'),
