use crate::{
    codec::*,
    limits::{reject_collection_growth, validate_label_input, MAX_IDENTITIES},
    Hydra, HydraMsgError, HydraResult,
};
...
impl Hydra {
    ...
    pub fn rename_id(&mut self, id: IdentityId, label: impl Into<String>) -> HydraResult<()> {
        ...
        validate_label_input(&label, "identity label size")?;
        let record = self
            .identities
            .get_mut(&id)
            .ok_or(HydraMsgError::IdentityNotFound)?;
    ...
    pub fn change_id_password(
        ...
        let record = self
            ...
            .get_mut(&id)
            .ok_or(HydraMsgError::IdentityNotFound)?;
        rewrap_identity_record(record, old_password.as_ref(), new_password.as_ref())?;
