...
- Any facade benchmark that includes local persistence or identity unlock/generation should be regenerated after `state-v1.hydra` encryption-at-rest and scrypt password KDF hardening, because normal facade writes and password checks now include memory-hard derivation work.
