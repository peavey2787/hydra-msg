...
On Windows, same-directory commit replacement uses the narrow `hydra-platform` FFI boundary and `MoveFileExW` with `MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH`. The destination is never explicitly deleted before replacement, so an existing committed state file is not exposed to the former delete-then-rename gap. The temporary file is fully written and `sync_all()` completes before the native replacement call. A Windows-only regression test proves replacement of an existing destination and removal of the temporary source. As on other platforms, filesystem and storage hardware failure semantics remain subject to the operating system and device; HYDRA claims native replace-existing semantics, not immunity to arbitrary power-loss or hardware faults.
...
Before a release claim, run the full master validation path on a machine with Rust, wasm-pack tooling, Node, and a real browser. Capture fresh desktop and mobile browser results from `examples/mobile_perf_web` and record them in [Benchmark notes](../validation/benchmarks/benchmark-results.md).
