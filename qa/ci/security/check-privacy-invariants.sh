...
contact_file="crates/hydra-msg/src/api/contacts.rs"
contact_codec_file="crates/hydra-msg/src/codec/contacts.rs"
lobby_file="crates/hydra-msg/src/api/lobbies.rs"
lobby_delivery_file="crates/hydra-msg/src/lobby/delivery.rs"
lobby_routing_file="crates/hydra-msg/src/lobby/routing.rs"
lobby_codec_file="crates/hydra-msg/src/codec/lobbies.rs"
auth_file="crates/hydra-msg/src/api/anonymous_auth.rs"
...
if [ ! -f "$storage_file" ] || [ ! -f "$storage_codec_file" ] || [ ! -f "$encrypted_snapshot_file" ] || [ ! -f "$snapshot_file" ] || [ ! -f "$identity_codec_file" ] || [ ! -f "$kdf_codec_file" ] || [ ! -f "$lib_file" ]; then
  echo "hydra-msg storage/KDF files missing" >&2
  exit 1
fi
if [ ! -f "$contact_file" ] || [ ! -f "$contact_codec_file" ] || [ ! -f "$lobby_file" ] || [ ! -f "$lobby_delivery_file" ] || [ ! -f "$lobby_routing_file" ] || [ ! -f "$lobby_codec_file" ]; then
...
require_source_text "$encrypted_snapshot_file" "encode_encrypted_state" "normal state is sealed before writing"
require_source_text "$encrypted_snapshot_file" "decode_encrypted_state" "normal state is opened with authentication"
...
require_source_text "$lobby_delivery_file" "let routing_hint = HydraLobbyRoutingHint::from_bytes(random_array::<32>()?)" "lobby routing hints are randomized per encrypted copy"
...
require_source_text "$snapshot_file" "anonymous_auth_secret" "anonymous auth issuer secret is stored inside encrypted state snapshot"
require_source_text "$snapshot_file" "anonymous_auth_spent" "anonymous auth spent nullifiers are stored inside encrypted state snapshot"
