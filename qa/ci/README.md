...
| `fuzz/check-fuzz.ps1` / `fuzz/check-fuzz.sh` | Bounded deterministic fuzz-smoke gate plus nightly cargo-fuzz/libFuzzer release campaigns. Standalone deterministic runs use `cargo run --locked`; normal bounded GitHub CI keeps its existing `HYDRA_CI_EPHEMERAL_LOCK_REFRESH=1` path. Local and release fuzz validation use the committed root lock strictly with `--locked`; only the bounded GitHub CI compatibility path may resolve ephemerally. The scripts propagate `HYDRA_FUZZ_TOOLCHAIN` (default `nightly`) into cargo-fuzz's nested Cargo build. Smoke mode defaults to 256 runs per target; overnight mode is time-bounded; deep mode uses 100,000 runs for fast targets and 1,000 for the slow stateful target. |
